 ...
 * TURBO GENERATORS
 *
 * Custom generators for scaffolding new apps and shared libraries
 *
 * Available generators:
 * - `turbo gen app` - Create a new app with domain, service, repository, eden-treaty, and config packages
...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
  ...
  plop.setGenerator('app', {
    description:
      'Create a new application with domain, service, repository, eden-treaty, and config packages',
    prompts: [
      {
        type: 'input',
        name: 'name',
      ...
      },
      {
        ...
        choices: [
          { name: 'TanStack Start - Full-stack with SSR and Eden Treaty', value: 'tanstack' },
          { name: 'Elysia Server - Standalone API with WebSocket support', value: 'elysia' },
        ],
      },
    ],
    actions: (answers) => {
      const appType = answers?.type as 'tanstack' | 'elysia'

      const actions: PlopTypes.ActionType[] = []

      // Determine template folder based on app type
      const templateFolder = appType === 'tanstack' ? 'app-tanstack' : 'app-elysia'
      ...
      // 5. For TanStack apps, create eden-treaty package in packages/{appName}/eden-treaty/
      if (appType === 'tanstack') {
        actions.push({
          type: 'addMany',
          destination: '{{ turbo.paths.root }}/packages/{{ name }}/eden-treaty',
          base: `templates/${templateFolder}/eden-treaty`,
          templateFiles: `templates/${templateFolder}/eden-treaty/**/*`,
          globOptions: { dot: true },
        })
      }
